...
try {
    ...
    error_log("Erreur PDO dans product-edit : " . $e->getMessage());
}
...
try {
    ...
    $product_size_ids = array_column($product_sizes, 'size_id'); // Liste des size_id actuels
} catch (PDOException $e) {
    ...
    if (empty($sneaker_name) || $brand_id <= 0 || $category_id <= 0 || $price <= 0 || $stock_quantity < 0) {
        ...
        try {
            ...
            $stmt_update->execute([
                ...
                'price' => $price ?? 0.0,
                'discount_price' => $discount_price, // Peut être null
                'stock_quantity' => $stock_quantity ?? 0,
                'description' => $description ?? '',
                'release_date' => $release_date, // Peut être null
                'is_new_arrival' => $is_new_arrival ?? 0,
            ...
            // Gestion de l'image principale (remplacement si une nouvelle est téléversée)
            if (!empty($_FILES['primary_image']['name'])) {
                if ($_FILES['primary_image']['error'] === UPLOAD_ERR_OK) {
                    ...
                    // Téléverser la nouvelle image principale
            ...
            if (isset($_POST['delete_images']) && is_array($_POST['delete_images'])) {
                foreach ($_POST['delete_images'] as $image_id) {
                    ...
                    if ($image && !$image['is_primary']) { // Ne pas supprimer l'image principale ici
            ...
            error_log("Erreur dans product-edit : " . $e->getMessage());
        }
    }
}
