...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    try {
        $db->beginTransaction();
        // Supprimer les détails de la commande (order_items) d'abord si nécessaire
        ...
        error_log("Erreur PDO dans orders : " . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id']) && isset($_POST['order_status'])) {
    ...
    $order_status = in_array($_POST['order_status'], ['pending', 'processing', 'shipped', 'delivered', 'cancelled']) ? $_POST['order_status'] : null;
    if ($order_status) {
        try {
                        ...
                        <?php foreach ($orders as $order): ?>
                            <tr>
                                ...
                                <td class="actions-cell">
                                    ...
                                    <a href="orders.php?delete=<?= $order['order_id'] ?>" class="btn-action delete-btn" title="Supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette commande ?');">
