<?php
// Page des sneakers pour enfants
require_once 'includes/config.php';
require_once 'includes/functions.php';

// Filtres
$filters = ['gender' => 'enfant'];
...
$sql = "SELECT COUNT(*) as total FROM sneakers s WHERE gender = 'enfant'";
$params = [];
...
$page_title = "Sneakers pour Enfants - Bander-Sneakers";
$page_description = "Découvrez notre collection de sneakers pour enfants. Des modèles colorés et confortables pour les petits pieds.";
...
<section class="category-banner">
    <div class="container">
        <div class="category-banner-content">
            <h1 class="category-title">Sneakers Enfants</h1>
            <p class="category-description">Découvrez notre collection de sneakers pour enfant. Des modèles confortables et stylés, parfaits pour accompagner leurs aventures au quotidien.</p>
        </div>
    </div>
</section>
        ...
        <div class="product-grid-container">
            ...
            <div class="shop-sidebar">
                <div class="filter-widget">
                    <h3>Filtres</h3>
                    <form action="enfants.php" method="GET" class="filter-form">
                        <?php
            ...
            <div class="product-grid-section">
                ...
                <div class="listing-header">
                    ...
                    <label for="sort-by">Trier par:</label>
                        <select name="sort" id="sort" onchange="window.location.href=this.value">
                            <option value="enfants.php?<?= updateQueryString(['sort' => 'newest']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'newest') ? 'selected' : '' ?>>Les plus récents</option>
                            <option value="enfants.php?<?= updateQueryString(['sort' => 'price_asc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'price_asc') ? 'selected' : '' ?>>Prix croissant</option>
                            <option value="enfants.php?<?= updateQueryString(['sort' => 'price_desc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'price_desc') ? 'selected' : '' ?>>Prix décroissant</option>
                            <option value="enfants.php?<?= updateQueryString(['sort' => 'name_asc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'name_asc') ? 'selected' : '' ?>>A-Z</option>
                            <option value="enfants.php?<?= updateQueryString(['sort' => 'name_desc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'name_desc') ? 'selected' : '' ?>>Z-A</option>
                        </select>
                    </div>
                ...
                <?php if (empty($sneakers)): ?>
                    <div class="no-products">
                        ...
                        <a href="enfants.php" class="btn btn-primary">Voir tous les produits</a>
                    </div>
                <?php else: ?>
                        ...
                        <div class="pagination">
                            <ul>
                                <?php if ($current_page > 1): ?>
                                    <li>
                                        <a href="enfants.php?<?= updateQueryString(['page' => $current_page - 1]) ?>" aria-label="Page précédente">
                                ...
                                if ($start_page > 1) {
                                    echo '<li><a href="enfants.php?' . updateQueryString(['page' => 1]) . '">1</a></li>';
                                    if ($start_page > 2) {
                                ...
                                for ($i = $start_page; $i <= $end_page; $i++) {
                                    ...
                                    echo '<li class="' . $active . '"><a href="enfants.php?' . updateQueryString(['page' => $i]) . '">' . $i . '</a></li>';
                                }
                                ...
                                if ($end_page < $total_pages) {
                                    ...
                                    echo '<li><a href="enfants.php?' . updateQueryString(['page' => $total_pages]) . '">' . $total_pages . '</a></li>';
                                }
                                ?>

                                <?php if ($current_page < $total_pages): ?>
                                    <li>
                                        <a href="enfants.php?<?= updateQueryString(['page' => $current_page + 1]) ?>" aria-label="Page suivante">
...
function removeParamFromUrl($param) {
    ...
    return 'enfants.php' . ($query ? '?' . http_build_query($query) : '');
}
