...
if (!isset($_SESSION['order_id'])) {
    error_log("Order ID not set in session.");
...
// Ajouter un journal pour vérifier l'état de la session
...
try {
    ...
    // Ajouter un journal pour vérifier les détails de la commande
    error_log("Order details: " . print_r($order, true));

    // Déterminer le nom à afficher
    $displayName = 'Client';
    if (!empty($order['first_name'])) {
        $displayName = htmlspecialchars($order['first_name']); // Utilisateur connecté
    } elseif (isset($_SESSION['guest_first_name'])) {
        $displayName = htmlspecialchars($_SESSION['guest_first_name']); // Invité
    } else {
        error_log("No first_name found for order_id: $orderId");
    }

    // Récupérer les articles de la commande (uniquement pour cet order_id)
    $stmt = $db->prepare("
        SELECT oi.*, s.sneaker_name, s.price, s.discount_price, s.primary_image AS image_url, sz.size_value, sz.size_type, b.brand_name
        FROM order_items oi
    ...
    // Vérifier si des articles ont été trouvés pour cette commande
    if (empty($orderItems)) {
...
<!-- Le reste du HTML/CSS/JavaScript reste identique -->
...
unset($_SESSION['guest_first_name']); // Nettoyer les données de l'invité
