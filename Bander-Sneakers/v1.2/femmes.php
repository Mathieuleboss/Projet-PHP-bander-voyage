...
$sql = "SELECT COUNT(*) as total FROM sneakers s WHERE gender = 'femme' OR gender = 'unisex'";
$params = [];
...
$page_title = "Sneakers pour Femme - Bander-Sneakers";
$page_description = "Découvrez notre collection de sneakers pour femme. Des modèles iconiques aux dernières tendances, trouvez la paire parfaite pour sublimer votre style.";
...
<section class="category-banner">
    <div class="container">
        <div class="category-banner-content">
            <h1 class="category-title">Sneakers Femme</h1>
            <p class="category-description">Découvrez notre collection de sneakers pour femme. Des modèles iconiques aux dernières tendances, trouvez la paire parfaite pour sublimer votre style.</p>
        </div>
    </div>
</section>
        ...
        <div class="product-grid-container">
            ...
            <div class="shop-sidebar">
                <div class="filter-widget">
                    <h3>Filtres</h3>
                    <form action="femmes.php" method="GET" class="filter-form">
                        <?php
            ...
            <div class="product-grid-section">
                ...
                <div class="listing-header">
                    ...
                    <div class="sorting">
                        ...
                        <select name="sort" id="sort" onchange="window.location.href=this.value">
                            <option value="femmes.php?<?= updateQueryString(['sort' => 'newest']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'newest') ? 'selected' : '' ?>>Les plus récents</option>
                            <option value="femmes.php?<?= updateQueryString(['sort' => 'price_asc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'price_asc') ? 'selected' : '' ?>>Prix croissant</option>
                            <option value="femmes.php?<?= updateQueryString(['sort' => 'price_desc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'price_desc') ? 'selected' : '' ?>>Prix décroissant</option>
                            <option value="femmes.php?<?= updateQueryString(['sort' => 'name_asc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'name_asc') ? 'selected' : '' ?>>A-Z</option>
                            <option value="femmes.php?<?= updateQueryString(['sort' => 'name_desc']) ?>" <?= (isset($filters['sort']) && $filters['sort'] == 'name_desc') ? 'selected' : '' ?>>Z-A</option>
                        </select>
                    </div>
                ...
                <?php if (empty($sneakers)): ?>
                    <div class="no-products">
                        ...
                        <a href="femmes.php" class="btn btn-primary">Voir tous les produits</a>
                    </div>
                <?php else: ?>
                        ...
                        <div class="pagination">
                            <ul>
                                <?php if ($current_page > 1): ?>
                                    <li>
                                        <a href="femmes.php?<?= updateQueryString(['page' => $current_page - 1]) ?>" aria-label="Page précédente">
                                ...
                                if ($start_page > 1) {
                                    echo '<li><a href="femmes.php?' . updateQueryString(['page' => 1]) . '">1</a></li>';
                                    if ($start_page > 2) {
                                ...
                                for ($i = $start_page; $i <= $end_page; $i++) {
                                    ...
                                    echo '<li class="' . $active . '"><a href="femmes.php?' . updateQueryString(['page' => $i]) . '">' . $i . '</a></li>';
                                }
                                ...
                                if ($end_page < $total_pages) {
                                    ...
                                    echo '<li><a href="femmes.php?' . updateQueryString(['page' => $total_pages]) . '">' . $total_pages . '</a></li>';
                                }
                                ?>

                                <?php if ($current_page < $total_pages): ?>
                                    <li>
                                        <a href="femmes.php?<?= updateQueryString(['page' => $current_page + 1]) ?>" aria-label="Page suivante">
...
function removeParamFromUrl($param) {
    ...
    return 'femmes.php' . ($query ? '?' . http_build_query($query) : '');
}
